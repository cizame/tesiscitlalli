...
\item
El inverso de $\phi_x$ es $\phi_{x^{-1}}$.
\begin{equation*}
  \begin{split}
...
$\Rightarrow$) Sean $H$ un grupo y $T$ un conjunto que cumple con las
condiciones de la definicion~\ref{defcay}; es decir, como hipótesis se
tiene que la grafica $G=\Cay(H,T)$, es de Cayley. 

Sea $A$ el grupo de automorfismos de la gráfica $G$.


Como $G$ es de Cayley, por el teorema~\ref{CayVT}, es
vertice-transitiva. Entonces para cada $w \in H=V(G)$, existe un
automorfismo $\sigma_v$ tal que al vertice $w$
lo manda al vertice $vw$, para todo $v \in H=V(G)$. Defínase:
\begin{equation*}
A_w:=\{
\sigma_{v} \in A \mid \sigma_{v} (w)=vw, v\in V(G)\}\end{equation*}

\begin{enumerate}

\item $A_w$ es un subgrupo de $A$.

\begin{enumerate}

\item La identidad de $A$ está en $A_w$. 

La identidad de $A$ es el automorfasmo que deja fijos a todos los
vertices, en particular deja fijo a $w$; o en otras palabras manda a
$w$ en $ew$, con $e$ la identidad de $H$.

Entonces 
Por hipotesis $G$ es gráfica de Cayley, entonces sus
vértices son los elementos del grupo $H$. Como $e\in H$ entonces $\sigma_e(w)=ew=w$.

Ahora, si $\sigma_{v_1}, \sigma_{v_2} \in A_w$ entonces
...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
por lo que $\sigma_{v_1}\circ \sigma_{v_2} \in A_w$.
\end{enumerate}

Por lo tanto $A_w$ es un subgrupo de $A$, 
\item pero falta verificar que
actúa regularmente.
