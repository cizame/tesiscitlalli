...
\newcommand{\bea}{\begin{eqnarray}} %Para centrar y enumerar ecuaciones
  \newcommand{\eea}{\begin{eqnarray}} \newcommand\N{{\mathbb N}}
    ...
    matemáticas y no solo aquí sino en muchas otras ciencias; pues
    teoría de gráficas no es la excepción. Dentro de ella existe un
    problema muy conocido por quienes se dedican a esto, la búsqueda
    de jaulas, el cual consiste en encontrar gráficas regulares, con
    cuello dado, pero con la menor cantidad de vértices posibles.


    La presente tesis aborda el problema de las jaulas 3$-$regulares
    de cuello dado. Primero se hace un recorrido por dichas jaulas
    pero de cuellos pequeños, de las cuales ya se ha demostrado la
    cantidad de vértices que tienen y si son únicas o no. Si el cuello
    es mayor o igual a trece, lo que se tiene, son solo cotas
    inferiores y superiores, pero aun no se ha podido demostrar si las
    gráficas existentes son jaulas y mucho menos si son únicas.  La
    segunda parte de la tesis, presenta un método computacional para
    dibujar gráficas de cuello par, a partir de un listado de grupos,
    para finalmente identificar si alguna reduce la cota superior.


    El método que se sigue, en la segunda parte, tiene dos niveles; el
    primero, es construir gráficas de Cayley localmente $3K_2$,
    $\Cay(G,T)$, las cuales son vértice-transitivas, y basta con hacer
    un análisis al rededor de la identidad para saber como se comporta
    en el resto de los vértices. El segundo paso consiste en construir
    la gráfica bipartita clanica $\Gamma$, a partir de $\Cay(G,T)$; en
    dicha gráfica, por cada vértice y por cada clan de $\Cay(G,T)$,
    existe un vértice en $\Gamma$ asociado a \'el, en donde los
    vértices $x,y$ son adyacentes si y solo si $x$ es un vértice de
    $\Cay(G,T)$ y $y$ un clan que contenga a $x$ o viceversa.

    Para construir gráficas de Cayley es necesario trabajar con
    grupos, y dado que $GAP$ es un software libre que contiene una
    amplia lista de ellos, se utiliza para hacer el programa que
    construya las gráficas de Cayley y a partir de estas construya las
    bipartitas clánicas, para finalmente reportar el cuello y el
    numero de vértices de estas.
    ...
    Antes de comenzar con el tema principal del presente trabajo,
    ser\'a necesario introducir algunos conceptos, unos mas básicos y
    comunes que otros, pero necesarios para facilitar la comprensión
    del lenguaje que aquí se maneja; así como resultados ya existentes
    que servirán de ayuda en la justificación del análisis que se hace
    de las gráficas 3-regulares.

 
    \section{Grupos}

    En esta sección se encuentran definiciones básicas de grupos y
    algunos resultados elementales que no serán demostrados, pero si
    el lector se interesa puede consultar m\'as acerca del tema en
    $\cite{Ehrlich}$, $\cite{Barrera}$ y $\cite{Zaldivar}$.

    \begin{definition}{\textbf{Grupo}}\\\index{Grupo} Un grupo es una
      pareja ($G$,$\circ$), con $G$ un conjunto no vacio, $\circ$ una
    ...
    Apartir de este momento, la notacion $g_1\circ g_2$ ser\'a
    sustituida por $g_1g_2$ pues es m\'as comodo dejar de escribir
    $\circ$.
...
Demostración: Como $g\in G$ entonces existe un elemento $g'\in G$ tal
...
y $n$ a $1$, con $j=1,2,...,n-1$, \'esta permutación se denota como
$(12...n)$; una misma permutación tiene varias representaciones pues
$(23...n1)$ es igual a $\sigma$.
...
\begin{definition}\textbf{Subgrupo}\\\index{Subgrupo}
  Sea $G$ un grupo y $H$ un conjunto no vacio de $G$. Se dice que $H$
  es un subgrupo si la operación de $G$ restringida a $H$ hace de
  \'este un grupo y denota como $H\le G$.
\end{definition}
...
para fines de esta tesis solo se consideran este tipo de grupos, a los
...
\mid g\in G \}$ y se denota pro $[G:H]$; a $Hg$ se le conoce como la
...
\begin{example}
  ...
  \item La accion es transitiva
    \begin{enumerate}
    \item Para $1,2\in X$, la permutacion $(123)$ manda $1$ a $2$ y la
      permutacion $(132)$ manda $2$ a $1$.
...
este resultado no ser\'a demostrado aqu\'i.

\section{Gráficas}

En la sección anterior se habl\'o acerca de grupos, una rama de las
matemáticas bastante mas común que la presente sección. Seguramente
...
interesa en conocer m\'as acerca de teoría de gráficas puede consultar
$\cite{Harary}$.
...
La definición general de una gráfica no es esta, pues en ella no se
requiere que $V(\Gamma)$ sea finito, y $E(\Gamma)$ es definido de otra
forma de tal manera que sus elementos pueden ser
dirigidos\index{Arista dirigida}, incluso pueden existir
lazos\index{Lazos}, los cuales son aristas que van de un vértice a si
mismo; pero para fines de esta tesis es suficiente definirla así:
vértices finitos, aristas no dirigidas y sin lazos.

\begin{definition}\textbf{Complemento de una gráfica}\\\index{Complemento de una gráfica}
  Si $G$ es una gráfica, se define $\overline{G}$, el complemento de
  $G$ como:
...
Las gráficas se representan frecuentemente en forma de un dibujo, pero
...
hablando de la misma gráfica; para detectarlo, se dan las siguientes
definiciones.
...
\begin{proposition}\textbf{Grupo de Automorfismo de una gráfica}\\\index{Grupo de automorfismos}
  Si $|\Gamma|=n$ entonces, el conjunto de todos los automorfismos de
  $\Gamma$, es un subgrupo de $S_n$, denotado $\Aut(\Gamma)$, llamado
  grupo de automorfismos de $\Gamma$.
\end{proposition}

Demostración: Los automorfismos de $\Gamma$ pertenecen a $S_n$ y la
identidad es un automorfismo. Sean $\phi$ y $\psi$ automorfismos de
$\Gamma$ y sean $x,y\in V(\Gamma)$ tales que $x\sim y$ entonces
$\phi(x)\sim \phi(y)$ y ambos están en $V(\Gamma)$, aplicando $\psi$:
\begin{equation*}
...
por lo que $(\psi \circ \phi)$ es también un automorfismo de $\Gamma$,
por lo tanto el grupo de automorfismos de $\Gamma$ es un subgrupo de
$S_n$.
...
\begin{definition}\textbf{Grafica $P_n$}\\\index{$P_n$}
  Los vertices de la grafica $P_n$ son $V(P_n):=\{v_1,v_2,...,v_n\}$ y
  las aristas $E(P_n):=\{v_i \sim v_{i+1} \mid i<n\}$. La
  figura~\ref{P5} es la grafica $P_5$.
\end{definition}
...
La union de $G$ con $G$ se denota por $2G$. En general la union
disjunta de $G$, n veces, se denota $nG$. \index{nG}
...
En otras palabras, la gráfica bipartita clánica es aquella cuyos
...
\begin{definition}\textbf{Subgráfica inducida}\\\index{Subgráfica inducida}
  La subgráfica $H$ de $G$ se dice inducida si $\{x,y\}\in
  V(H)^{(2)}\cap E(G)$ entonces $\{x,y\}\in E(H)$.
\end{definition}
...
Demostración: Sea $\varphi : G \rightarrow H$ un isomorfismo entre G y
...
\begin{definition}\textbf{Camino de longitud $n$}\\\index{Camino de longitud $n$}
  Un camino de longitud $n$, en $G$, es una sucesión de vértices de
...
entonces $\{v=t_n,...,t_1,t_0=u \}$ es un $vu$-camino. Ahora se
demostrar\'a que es una relación de equivalencia.
...
Los \'arboles o la unión disjunta de ellos, son las únicas gráficas
...
Sea $x\in V(G)$ y $a,b,c$ los vecinos de $x$ tales que $a\sim b\sim c$
para que la vecindad de $x$ sea isomorfa a $P_2$, al hacer esto,
automáticamente la vecindad de $b$ también es $P_2$.

Sin perdida de generalidad se elige $a$ para tratar de completar su
vecindad, para ello $a$ tiene que tener otro amigo, nótese que dicho
amigo no puede ser $c$, pues esto arruinaría la vecindad de $x$;
entonces debe ser un nuevo vértice $d$, que a su vez debe estar unido
a $b$ para que la vecindad sea $P_2$, pero eso ahora arruina la
vecindad de $b$, figura~\ref{isomorfp2}.
...
tiene ciclos de longitud impar. Sea $v_1, v_2,..., v_m, v_1$ un ciclo
de $G$, al ser bipartita, sabemos que podemos colorear con dos
...
\begin{theorem}\textbf{}\\
  Si $G$ es una gráfica conexa, con $ |G| \geq 2$ y no tiene ciclos
...
\begin{definition}\textbf{Grado de un vértice}\\\index{Grado de un vértice}
  Sea $G$ una gráfica, $x\in V(G)$, el grado de $x$,
  $d_G(x)$\index{$d_G(x)$}, es $|N_G(x)|=|V(N_G(x))|$.
\end{definition}

En otras palabras, el grado de un vértice coincide con la cantidad de
amigos que tiene.

\begin{definition}\textbf{Grado mínimo de una gráfica}\\\index{Grado mínimo de una gráfica}
  El grado mínimo de $G$ \index{$\delta(G)$} es $\delta(G)=\min
  \{d_G(x)\mid x\in V(G)\}$.
\end{definition}
...
Demostración: Consideremos la gráfica bipartita, $B(G)$, con vértices
...
Demostración: Sea $m$ la cantidad de vértices de grado impar. Si en la
...
Demostración: Sea $v_0 \in V(G)$, consideremos el $v_0v_n$-camino, un
...
Demostración: Como $G$ es $2$-regular, por el teorema~\ref{teo8},
entonces tiene un ciclo y como es conexa entonces el ciclo abarca
todos los vértices de $G$ por lo tanto $G \cong C_n$.

\begin{corollary}\textbf{}\\
  Si $G$ es una gráfica $2$-regular, entonces $G=C_{n_1}\cup C_{n_2}
  \cup ... \cup C_{n_k}$ donde $|G|=n_1 + ...+ n_k$.
\end{corollary}
...
\begin{theorem}\textbf{}\\
  Si $G$ es cubica y $|G|=6$ entonces $G$ es isomorfa a la bipartita
  completa $K_{3,3}$ o $G$ es isomorfa a $\overline{C_6}$.
\end{theorem}
...
Observación: La gráfica $\Gamma$ es vértice-transitiva si el grupo de
automorfismos de $G$, $\Aut(\Gamma)$, actúa transitivamente en
$V(\Gamma)$.
...
Demostración: Sean $x,y \in V(G)$ y sea $\phi: G \rightarrow G $ un
...
\begin{definition}\textbf{Gráfica localmente $3K_2$}\\\index{Gráfica localmente $3K_2$}
  ...
  constante, con $N_G(x)\cong 3K_2$, para todo $x\in V(G)$.
\end{definition}
...
El termino jaula, es uno de los m\'as usados en la presente, pues es
precisamente el objetivo principal, y ha llegado la hora de definirlo.
...
Los ciclos son un ejemplo de $(2,g)$-gráficas y las completas $K_n$,
lo son de $(n-1,3)$-gráficas; pero la unión disjunta de dos o m\'as de
ellas, de cada conjunto respectivamente, también lo son.

Para definir una $(k,g)$-gráfica no fue necesario mencionar la
cantidad de vértices que deban tener o si son conexas o no, como lo ha
hecho notar el ejemplo anterior.
...
natral que surge es: ¿Cu\'al es la menor cantidad de vértices que una
$(k,g)$-gráfica tiene?.
...
Entonces la pregunta ser\'ia ¿Cu\'antos vértices tiene una
...
$m-1$ vértices, y son las gráficas $k_n$. Pero en general la respuesta
a esta interrogante no es conocida.

Con $m=3$ y $n=3,4,...,12$, las jaulas ya son conocidas y la
descripción de estas se presentan adelante en otra sección. Para
$n\geq 13$ las graficas aun no son conocidas, lo que se tiene son solo
...
\begin{definition}\textbf{Gráfica de Cayley}\\\index{Gráfica de Cayley}
  ...
  es adyacente a $g_2$ si y solo si $g_2^{-1}g_1 \in T$.
\end{definition}
...
Si $T=\{t_1,t_2,t_3,..., t_n\}$, entonces los vecinos de $g\in G$, en
$\Cay(G,T)$, son $gt_1,gt_2,...,gt_n$, pues si $x$ es vecino de $g$,
entonces $g^{-1}x\in T$ entonces existe $t_i$ tal que $g^{-1}x=t_i$
entonces $x=gt_i$. Por otro lado, considerando $gt_i$, $g$ es vecino
de $gt_i$ si y solo si $g^{-1}(gt_i)\in T$, si y solo si $t_i \in T$.
\newline De lo anterior es evidente que en $\Cay(G,T)$, los vecinos de
$I \in G$ son exactamente los elementos de $T$. Hecho que es de suma
importancia para dar condiciones a los elementos de $T$, que hagan que
la gráfica sea localmente $3K_2$.


Otro resultado sumamente importante es el teorema~\ref{CayVT}, pues al
ser vértice-transitivas una gráfica, basta verificar como se comporta
al rededor de la identidad para conocer como es en cualquier otro
vértice.
...
Demostración: Para probar que la gráfica es vértice-transitiva, se
muestra que existe un automorfismo, tal que manda a un vértice en
otro, para cualesquiera par de vértices.

Sea $\Cay(G,T)$ y sean $g_1, g_2$ dos vértices de la gráfica. Dado $x
\in G$, defínase $\phi_x:G \rightarrow G$ tal que $g \rightarrow xg$,
...
En otras palabras, si $g_1$ y $g_2$ son cualesquiera dos vértices,
...
$\Rightarrow$) Como hipotesis se tiene $G=\Cay(H,T)$. Sea $A$ el grupo
...
La identidad esta en $A_s$, pues al ser gráfica de Cayley, sus
...
$d(u,u)=d(v,v)=0$ entonces por ser distancia-transitiva existe
$\phi\in Aut(\Gamma)$ tal que $\phi(u)=v$.  \newline Observación: Si
$\Gamma$ es distancia-transitiva entonces es 1-arco-transitiva. Pues
si $u\sim v$ y $x\sim y$ entonces $d(u,v)=d(x,y)=1$ entonces existe
$\phi \in Aut(\Gamma)$ tal que $\phi(u)=x$ y $\phi(v)=y$.
...
igual a catorce, pero para ello primero se dará un recorrido por las
...
falta dos aristas, $c$ y $d$, figura~\ref{arbol(3,6)}.
...
Sin perdida de generalidad, se puede comenzar a completar $c$. Para la
...
a la misma rama a la ves porque se formarían ciclos menores,
...
ingenua, y es la representación de la gráfica de puntos y lineas del
...
Hasta este momento el análisis a sido relativamente sencillo, pues al
ir descartando posibilidades se a podido decidir que forma tiene la
jaula y al mismo tiempo se demuestra que es única, mas aun parecería
...
\newline Si se desea saber mas sobre jaulas se puede consultar
...
\begin{lemma}\label{bamnocayley}
  Sean $a,b\in T$ tales que $a\sim b$ en la grafica $Cay(G,T)$;
...
ser del tipo uno. Despues, $b$ se puede escribir de la forma $b=a^n$
...
esto ocurre si y solo si
 ...
 teorema~\ref{orbitas}, basta calcular las orbitas del grupo de
 ...
 Sin embargo, computacionalmente el calcular orbitas es muy caro, pues
 ...
 elementos cuyo orden es tres, para poder hacer todas las
 combinaciones de tres elementos, generar $T$ y verificar que no
 cumpla las condiciones malas. Si esto ocurre diremos que dichas
 combinaciones generan una $T$ buena.\index{T buena}
 ...
 sus orbitas. Para que esto no ocurra es conveniente notar que en la
...
El cual al construir la bipartita clánica se vuelve un ciclo de
longitud $2n$.  \newline
...
$b$; lo que trae como consecuencia que $G$ se pueda generar a partir
de dos elementos, contradiciendo el hecho de que el n\'umero m\'inimo
de generadores es mayor o igual a tres.
...
uno, se genera en el grupo uno, con veintiun elementos, es decir
...
cuello diez, tiene ciento catorce vertices y se generó en el grupo
...
La gráfica cinco del primer grupo de doscientos tres elementos, genera
...
cuatro vértices. Aunque la grafica encontrada es mayor a la cota
actual; est\'a si iguala la cantidad de vertices, que hasta antes del
2003 existia, pues tambien tenia cuatrocientos seis vertices.
...
encontradas por este metodo, existe la posibilidad de que existan
