...
vertice-transitiva. Entonces para cada $v \in H=V(G)$, existe un
automorfismo $\phi_v$ tal que a cada vertice $w$
lo manda al vertice $vw$, la demostración de que $\phi_v$ es un
...
vertices de la grafica. En otras palabras, manda a
...
\item
El subgrupo $B$ actúa regularmente en el conjunto de vertices de la gráfica.
...
entonces $\phi_v$ es tal que manda el vertice $v_1$ al vertice $v_2$,
para toda pareja $\{v_1,v_2\}$.

Por lo tanto $B$ actúa transitivamente en $V(G)$.

\item \label{sab12}
El orden del estabilizador de $v$, en el sibgrupo $B$, es igual a uno, para todo
$v \in V(G)$. 
...
grupo de automorfasmos de la grafica $Cay(H,T)$,  actúa regularmente en
el conjunto de vertices $V(Cay(H,T))=H$. 

\smallskip

\item $\Leftarrow$) 

Sea $A_s$ el subgrupo del grupo de automorfismos de $G$
