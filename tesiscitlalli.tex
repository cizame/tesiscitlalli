...
matemáticas bastante m\'as común que la presente sección. Seguramente
...
La definición general de una gráfica permite que $V(\Gamma)$ sea infinito, y $E(\Gamma)$ es definido de tal forma que sus elementos pueden ser
dirigidos\index{Arista dirigida}; incluso pueden existir
lazos\index{Lazos}: aristas que van de un vértice en s\'i
mismo. Pero para fines de esta tesis es suficiente definirla con
...
\begin{proposition}\textbf{Grupo de Automorfismo de una gráfica}\\\index{Grupo de automorfismos}
  Sea $n$ el orden de una gráfica, $|\Gamma|=n$. Entonces, el conjunto de todos los automorfismos de
  $\Gamma$, es un subgrupo de $S_n$, denotado $\Aut(\Gamma)$, llamado
  grupo de automorfismos de $\Gamma$.
\end{proposition}

\begin{demostration} Los automorfismos de $\Gamma$ pertenecen a $S_n$ y la
identidad es un automorfismo, por lo que el conjunto de automorfismos de $\Gamma$ no es vac\'io. Sean $\phi$ y $\psi$ automorfismos de
...
por lo que $(\psi \circ \phi)$ es también un automorfismo de $\Gamma$. Por lo tanto el grupo de automorfismos de $\Gamma$ es un subgrupo de
$S_n$.
\end{demostration}
...
La unión de $G$ con $G$ se denota por $2G$. En general la unión
...
\end{proposition}
\begin{demostration} Sea $\{u=t_0,t_1,t_2,...,t_n=v \}$ un $uv$-camino
...
\begin{proposition}\textbf{}\\
  No existe una gráfica $G$ tal que la vecindad de $x$ sea isomorfa a $P_2$ para todo $ x\in
  V(G)$.
\end{proposition}
\begin{demostration} Para demostrar que no existe $G$ tal que $N_G(x)\simeq P_2$, se tratar\'a de dibujar ua gráfica con esta propiedad, para identificar en que
...
$n\geq 13$ las gráficas aun no son conocidas, lo que se tiene son solo
